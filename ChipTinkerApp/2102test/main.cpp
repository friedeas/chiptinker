...
void writeByteToMem(DeviceDriver * gerat, char data, int address)
{
    ...
    //dat is a data word for comparing
                                            //R/W
    QString state=addr.mid(6,1)+addr.mid(5,1)+"1"+addr.mid(1,1)+addr.mid(2,1)+addr.mid(3,1)+
            addr.mid(4,1)+addr.mid(0,1)+"01"+dat+"0"+"1"+addr.mid(9,1)+addr.mid(8,1)+addr.mid(7,1);
                            //        gnd Vcc DIN DOut /CE
    gerat->setData(state);
    gerat->qSleep(TIME_WAIT);

    state.replace(2,1,"0");
    state.replace(12,1,"0");
    gerat->setData(state); //bang CS/WE lines down
    gerat->qSleep(TIME_WAIT);

    state.replace(2,1,"1");
    state.replace(12,1,"1");
...
QString readByteFromMem(DeviceDriver * gerat, int address)
{
    ...
    QString state=addr.mid(6,1)+addr.mid(5,1)+"1"+addr.mid(1,1)+addr.mid(2,1)+addr.mid(3,1)+
            addr.mid(4,1)+addr.mid(0,1)+"01"+"0"+"0"+"0"+addr.mid(9,1)+addr.mid(8,1)+addr.mid(7,1);
                           //        gnd Vcc DIN DOut /CE
...
int main(int argc, char *argv[])
{
    ...
    //qsrand((uint)time.msec()); //this stops exist with qt6
