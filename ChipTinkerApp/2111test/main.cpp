...
void writeByteToMem(DeviceDriver * gerat, char data, int address)
{
    ...
    QString state=addr.mid(3,1)+addr.mid(2,1)+addr.mid(1,1)+addr.mid(0,1)+addr.mid(5,1)+
            addr.mid(6,1)+addr.mid(7,1)+"010"+dat+"0"+"1"+addr.mid(4,1)+"1";
    gerat->setData(state);
    gerat->qSleep(TIME_WAIT);

    state.replace(15,1,"0");
    gerat->setData(state); //bang RW line down
    gerat->qSleep(TIME_WAIT);

    state.replace(15,1,"1");
    gerat->setData(state); //bang RW line up
    gerat->qSleep(TIME_WAIT);
...
QString readByteFromMem(DeviceDriver * gerat, int address)
{
    ...
    QString state=addr.mid(3,1)+addr.mid(2,1)+addr.mid(1,1)+addr.mid(0,1)+addr.mid(5,1)+
            addr.mid(6,1)+addr.mid(7,1)+"00"+"0"+"0000"+"01"+addr.mid(4,1)+"1";
