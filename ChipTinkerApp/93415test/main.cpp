//Copyright 2014,2015..2020 MCbx, All rights reserved.
...
void writeByteToMem(DeviceDriver * gerat, char data, int address)
{
    ...
    //dat is a data word for comparing
               // /CS                                                                    //DOUT GND
    QString state="0"+addr.mid(0,1)+addr.mid(1,1)+addr.mid(2,1)+addr.mid(3,1)+addr.mid(4,1)+"0"+"0"+
            addr.mid(5,1)+addr.mid(6,1)+addr.mid(7,1)+addr.mid(8,1)+addr.mid(9,1)+"1"+dat+"1";
                            //                                                   /WE       Vcc
    gerat->setData(state);
    gerat->qSleep(TIME_WAIT);

    state.replace(13,1,"0");
    gerat->setData(state); //bang WE lines down
    gerat->qSleep(TIME_WAIT);

    state.replace(13,1,"1");
    gerat->setData(state); //bang WE lines up
    gerat->qSleep(TIME_WAIT);
...
QString readByteFromMem(DeviceDriver * gerat, int address)
{
    ...
    //           /CS                                                                       DOUT GND
    QString state="0"+addr.mid(0,1)+addr.mid(1,1)+addr.mid(2,1)+addr.mid(3,1)+addr.mid(4,1)+"0"+"0"+
            addr.mid(5,1)+addr.mid(6,1)+addr.mid(7,1)+addr.mid(8,1)+addr.mid(9,1)+"1"+"0"+"1";
                           //                                                     /WE Din Vcc
