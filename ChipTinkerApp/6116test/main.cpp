...
#define INITIAL_IO "000000001111111110000001"
...
void writeByteToMem(DeviceDriver * gerat, char data, int address)
{
    ...
    QString state=addr.mid(7,1)+addr.mid(6,1)+addr.mid(5,1)+addr.mid(4,1)+addr.mid(3,1)+addr.mid(2,1)+addr.mid(1,1)+addr.mid(0,1)
                   +dat.mid(0,1)+dat.mid(1,1)+dat.mid(2,1)+"0"+dat.mid(3,1)+dat.mid(4,1)+dat.mid(5,1)+dat.mid(6,1)+dat.mid(7,1)
                   +"1"+addr.mid(10,1)+"1"+"1"+addr.mid(9,1)+addr.mid(8,1)+"1";
                 // /CS                /OE /WE
       gerat->setData(state);
       gerat->qSleep(TIME_WAIT);

       state.replace(17,1,"0");
       state.replace(20,1,"0");
       gerat->setData(state); //bang CS/WE lines down
       gerat->qSleep(TIME_WAIT);

       state.replace(17,1,"1");
       state.replace(20,1,"1");
       gerat->setData(state); //bang CS/WE lines up
       gerat->qSleep(TIME_WAIT);

    //One more thing: I'm flipping CS and WE here together for a reason not to get outputs from data lines
    //because they're configured as inputs. This avoids chip abusing.
}
...
QString readByteFromMem(DeviceDriver * gerat, int address)
{
      ...
      state.replace(17,1,"0");
      state.replace(19,1,"0");
      gerat->setData(state); //bang CS/OE lines down
      gerat->qSleep(TIME_WAIT);
      ...
      state.replace(17,1,"1");
      state.replace(19,1,"1");
      gerat->setData(state); //bang CS/OE lines up
      gerat->qSleep(TIME_WAIT);

      //here is extracting data word from res to res :)
      res=res.mid(8,1)+res.mid(9,1)+res.mid(10,1)+res.mid(12,1)+res.mid(13,1)+res.mid(14,1)+res.mid(15,1)+res.mid(16,1);
